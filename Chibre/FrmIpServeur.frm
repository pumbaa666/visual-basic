...
Dim vTemp As String
    ...
    If vCntConnect = False Then
        FrmMain.Wsk.Connect ComboIP.Text, PortDistant
        vCntConnect = True
    Else
'        On Error GoTo Erreur
        vTemp = FrmMain.Wsk.State
        FrmMain.Wsk.SendData "[CONNECTED]"
        MsgBox "Connexion établie", vbInformation, "OK"
        FrmMain.Hide
'        FrmChat.Show

        Open "c:\chibre.ini" For Append As #1
        For vcnt = 0 To 50
...
Dim vCntIp As Integer
    ...
    Open "c:\chibre.ini" For Input As #1
    While Not (EOF(1))
        Line Input #1, vIp
            ComboIP.AddItem vIp
            tAllIP(vCntIp) = vIp
