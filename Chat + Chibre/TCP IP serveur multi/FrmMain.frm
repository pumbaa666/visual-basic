...
Private Sub ClkCarte_Timer()
Static vCntCarte As Integer

    If WSTCP(vCntCarte).State = 7 Then
'        WSTCP(vCntCarte).SendData tjoueur(vcntcarte,
    End If
End Sub
...
'            If TxtSend.Text = "salut" Then
'                RTB.SelStart = Len(RTB.Text) - Len(TxtSend.Text) - 1 - Len("admin :")
'                RTB.SelLength = Len(TxtSend.Text) + Len("admin :")
'                RTB.SelItalic = True
'            End If
'            RTB.Font.Italic = True
...
Private Sub CmdStart_Click()
    If vNbConnect - vDeconnect < 3 Then
        MsgBox "Il faut plus de joueur", vbCritical, "Erreur"
    Else
        vAffichage = "Lancement de la partie de chibre"
...
Private Sub LstConnected_KeyDown(KeyCode As Integer, Shift As Integer)
    If LstConnected.ListIndex > 1 And KeyCode = 46 Then
        ...
        vAffichage = LstConnected.Text & " s'est fait bannir"
        ClkEnvoie.Enabled = True
...
Dim vCount As Integer
    ...
    ElseIf Left(vData, 5) = "<new>" Then
        ...
        vAffichage = Right(vData, Len(vData) - 5) & " nous a rejoint"
        ClkEnvoie.Enabled = True
        ClkList.Enabled = True
    Else
        vAffichage = vData
