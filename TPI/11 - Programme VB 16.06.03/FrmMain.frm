...
Dim vNbDes As Integer
    ...
    vNbDes = NbPointsParDes(vNbTot)     ' Cette fonction sépare la valeur de chaque dés

    If vNbTot <> 0 Then     ' Il faut au moins un point
        If vNbDes <> FrmOption.TxtNbDes Then    ' Compare le nombre de dés affichés avec le nombre de dés spécifiés
            MsgBox "Le nombre de dés présents ne correspond pas à ce que vous avez spécifié!", vbCritical, "Erreur"
        Else
...
Dim vOver As Boolean
    ...
    On Error Resume Next
    '*** Empeche les "indice en dehors de la plage" ***'
    If vColonne < 5 Then
        vLimite = vColonne
    ...
    For vCntOver = vColonne - vLimite To vColonne + vLimite    ' Parcours, à la ligne du dessus, 5 colonnes avant et 5 colonnes après le pixel en train d'être testé
...
Function NbPointsParDes(ByVal vNbTot As Integer) As Integer
Const DISTANCE = 30
    ...
    Do
        ...
        Do
            If tCoo(0, vCountDes) <> 0 Then     ' Si il y a une coordonnée
                ...
                End If
                If tPointsParDes(vCountDes) = 0 Then
                    tPointsParDes(vCountDes) = 1 ' S'il n'y a qu'un seul point sur un dé
                    vNbDif = vNbDif + 1
                End If
            End If
